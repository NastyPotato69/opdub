...
def _try_decode_jpn(path: Path, verbose: bool = False) -> np.ndarray | None:
    """Decode jpn audio; return None (don't raise) if no jpn stream exists."""
    try:
        info = probe(path)
...
def cmd_align(args: argparse.Namespace) -> int:
    ...
    # Keep only files that have a jpn stream — eng-only dub files are skipped
    # here and used later for Stage 0.
    if verbose:
        print(f"Scanning {len(all_paths)} files in {source_dir}…")
